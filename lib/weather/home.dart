...
class _HomeState extends State<Home> {
  ...
  // Tạo danh sách mới
  List<dynamic> updatedWeatherList = [];

  // Lấy dữ liệu cụ thể của hôm nay và thêm vào danh sách mới
  for (int i = 0; i < 40; i++) {
    updatedWeatherList.add(consolidatedWeather[i]);
  }

  setState(() {
    // Cập nhật các giá trị mới
    temperatureF = (updatedWeatherList[0]["main"]["temp"]).round();
    weatherStateName = (updatedWeatherList[0]["weather"][0]["main"]);
    humidity = (updatedWeatherList[0]["main"]["humidity"]).round();
    windSpeed = (updatedWeatherList[0]["wind"]["speed"]).round();
    maxTempF = (updatedWeatherList[0]["main"]['temp_max']).round();

    // Định dạng hiển thị ngày
    var myDate =
        DateTime.parse((updatedWeatherList[0]["dt_txt"]).toString());
    ...
    consolidataWeatherList = updatedWeatherList;
  });
}
  @override
  void initState() {
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      var futurWeatherName =
                          consolidataWeatherList[index]["weather"][0]["main"];
                      var weatherUrl =
                          futurWeatherName.replaceAll('', '').toLowerCase();
                      var parsedDate = DateTime.parse(
                          (consolidataWeatherList[index]["dt_txt"]).toString());
                      // ignore: non_constant_identifier_names
                      var SlipStringDate = DateFormat('HH,EEEE')
                          .format(parsedDate)
                          .substring(0, 6);
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                "${(consolidataWeatherList[index]["main"]["temp"] - 273).round()}C",
                                style: TextStyle(
                                    fontSize: 17,
                                    color: selectedDay == today
                                        ? Colors.white
                              ...
                              Image.asset('${'assets/' + weatherUrl}.png',
                                  width: 40, alignment: Alignment.bottomCenter),
                              Text(
                                SlipStringDate,
                                style: TextStyle(
                                  fontSize: 16,
